...
type Cache[TKey comparable, TValue any] interface {
	...
	// GetWithRefresh returns the cached value and a flag indicating whether the entry is stale
	// and should be reloaded. Returns (value, true, nil) when the entry exists but needs refresh.
	// Returns ErrCacheMiss or ErrNegativeCacheHit when the entry does not exist.
