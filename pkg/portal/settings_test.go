...
func submitEmailChangeForTest(t *testing.T, srv *Server, cookie *http.Cookie, name, email string, code int) {
	t.Helper()
	_ = submitEmailChangeValueForTest(t, srv, cookie, name, email, strconv.Itoa(code))
}

func submitEmailChangeValueForTest(t *testing.T, srv *Server, cookie *http.Cookie, name, email, code string) *ViewModel {
	t.Helper()
	form := url.Values{}
	form.Set(common.ParamName, name)
	form.Set(common.ParamEmail, email)
	form.Set(common.ParamVerificationCode, code)
	req := httptest.NewRequest(http.MethodPut, "/settings/tab/general", strings.NewReader(form.Encode()))
	req.AddCookie(cookie)
	req.Header.Set(common.HeaderContentType, common.ContentTypeURLEncoded)
	viewModel, err := srv.putGeneralSettings(httptest.NewRecorder(), req)
	if err != nil {
		t.Fatal(err)
	}
	return viewModel
...
func TestPutGeneralSettingsEmailChallengeAttemptLimit(t *testing.T) {
	...
	for attempt := 0; attempt < maxFailedAttempts; attempt++ {
		viewModel := submitEmailChangeValueForTest(t, server, cookie, user.Name, newEmail, strconv.Itoa(wrongCode))
		if attempt == maxFailedAttempts-1 {
			renderCtx := viewModel.Model.(*settingsGeneralRenderContext)
			if renderCtx.TwoFactorError != verificationCodeError(true) {
				t.Fatalf("final failed attempt error = %q, want exhaustion", renderCtx.TwoFactorError)
			}
		}
	}
	submitEmailChangeForTest(t, server, cookie, user.Name, newEmail, code)
...
func TestPutGeneralSettingsMalformedCodeDoesNotRecordAttempt(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	...
	for _, malformed := range []string{"", "not-a-code", "-12345", "+12345", "12345", "1234567"} {
		viewModel := submitEmailChangeValueForTest(t, server, cookie, user.Name, newEmail, malformed)
		renderCtx := viewModel.Model.(*settingsGeneralRenderContext)
		if renderCtx.TwoFactorError != verificationCodeError(false) {
			t.Fatalf("malformed code %q error = %q", malformed, renderCtx.TwoFactorError)
		}
	}
	...
	var attempts int32
	if err := store.Pool.QueryRow(ctx, "SELECT failed_attempts FROM backend.sessions WHERE session_id = $1", sid).Scan(&attempts); err != nil {
		t.Fatal(err)
	}
	if attempts != 0 {
	...
	if updatedUser.Email != newEmail {
		t.Fatalf("valid code after malformed input left email as %q", updatedUser.Email)
	}
}
