...
func TestResend2fa(t *testing.T) {
	...
	stubMailer := server.Mailer.(*email.StubMailer)
	oldCode := stubMailer.LastCode

	req := httptest.NewRequest("POST", "/"+common.TwoFactorEndpoint+"/resend", nil)
	req.AddCookie(cookie)
	w := httptest.NewRecorder()
	srv.ServeHTTP(w, req)

	if w.Code != http.StatusOK {
		t.Errorf("Unexpected resend response code: %v", w.Code)
	}

	if stubMailer.LastCode == oldCode {
		t.Error("Code should have been regenerated")
	}
