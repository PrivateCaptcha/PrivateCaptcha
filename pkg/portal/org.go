...
func (s *Server) handlePortalError(orgID int32, err error, w http.ResponseWriter, r *http.Request) {
	if (orgID == -1) && (err == errNoOrgs) {
		...
		if _, revokeErr := s.Sessions.Revoke(w, r); revokeErr != nil {
			slog.ErrorContext(r.Context(), "Failed to revoke inconsistent session", common.ErrAttr(revokeErr))
...
func (s *Server) getOrgStats(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if errors.Is(err, db.ErrPermissions) {
	...
	if err != nil {
		...
		if errors.Is(err, db.ErrMaintenance) {
...
func (s *Server) getOrgSearch(w http.ResponseWriter, r *http.Request) (*ViewModel, error) {
	...
	searchTerm, err := normalizeOrgSearchTerm(r.URL.Query().Get(common.ParamQuery))
	if err != nil {
		return nil, err
	}
