...
func TestAuthoritativeSessionManagerSignInLifecycle(t *testing.T) {
	...
	if !ok || resolvedAuthority.State != session.StateAuthenticated || resolvedAuthority.UserID != userID || resolved.Get(req.Context(), session.KeyUserName) != "Signed In User" {
...
func TestAuthoritativeSessionManagerRegistrationLifecycle(t *testing.T) {
	...
	issueW := httptest.NewRecorder()
	issued, err := manager.IssueRegistrationChallenge(issueW, req, anonymous, registrationEmail, "111111", false, 15*time.Minute, 5)
	if err != nil || issued.Outcome != session.TransitionSucceeded || issued.Session == nil {
...
func TestSessionRegistrationTransitions(t *testing.T) {
	...
	issued, err := impl.IssueRegistrationChallenge(ctx, &dbgen.IssueRegistrationChallengeParams{
		...
		RequiresVerification: db.Bool(true), InviteID: db.Int(42), SessionTtl: 3 * time.Hour, ChallengeTtl: 15 * time.Minute, MaxAttempts: 3,
	})
	...
	required, err := impl.ConsumeRegistrationChallenge(ctx, &dbgen.ConsumeRegistrationChallengeParams{
		SessionID: sid, ChallengeCode: db.Text("111111"), MaxAttempts: 3,
	})
	...
	issued, err = impl.IssueRegistrationChallenge(ctx, &dbgen.IssueRegistrationChallengeParams{
		...
		RequiresVerification: db.Bool(false), InviteID: db.Int(42), SessionTtl: 3 * time.Hour, ChallengeTtl: 15 * time.Minute, MaxAttempts: 3,
	})
	...
	if _, err := impl.IssueRegistrationChallenge(ctx, &dbgen.IssueRegistrationChallengeParams{
		...
		RequiresVerification: db.Bool(false), SessionTtl: 3 * time.Hour, ChallengeTtl: 15 * time.Minute, MaxAttempts: 2,
	}); err != nil {
		t.Fatal(err)
	}
