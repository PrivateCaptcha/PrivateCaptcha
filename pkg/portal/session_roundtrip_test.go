...
func (q *countingSessionQuerier) ConsumeSignInChallenge(ctx context.Context, arg *dbgen.ConsumeSignInChallengeParams) (*dbgen.ConsumeSignInChallengeRow, error) {
	q.calls.Add(1)
	return q.Querier.ConsumeSignInChallenge(ctx, arg)
}

func (q *countingSessionQuerier) ConsumeEmailChangeChallenge(ctx context.Context, arg *dbgen.ConsumeEmailChangeChallengeParams) (*dbgen.ConsumeEmailChangeChallengeRow, error) {
	q.calls.Add(1)
	return q.Querier.ConsumeEmailChangeChallenge(ctx, arg)
}

func (q *countingSessionQuerier) ConsumeRegistrationChallenge(ctx context.Context, arg *dbgen.ConsumeRegistrationChallengeParams) (*dbgen.ConsumeRegistrationChallengeRow, error) {
	q.calls.Add(1)
...
func (q *countingSessionQuerier) RenewSessionExpirations(ctx context.Context, arg *dbgen.RenewSessionExpirationsParams) ([]*dbgen.RenewSessionExpirationsRow, error) {
	q.calls.Add(1)
...
func TestCriticalSessionAuthorityRoundtripBudgets(t *testing.T) {
	...
	t.Run("Warm private write", func(t *testing.T) {
		...
		localServer := *server
		localServer.Sessions = f.manager
		handled := false
	...
	t.Run("Cold sign-in consume", func(t *testing.T) {
		f := newRoundtripFixture(t)
	...
	for _, tt := range []struct {
		...
		{name: "Warm exhausted resend", warm: true, want: 2},
		{name: "Cold exhausted resend", want: 3},
	} {
