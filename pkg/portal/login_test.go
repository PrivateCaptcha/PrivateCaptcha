...
func TestPostLoginPreservesReturnURL(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	...
	const returnURL = "/settings?source=login"
	privateReq := httptest.NewRequest(http.MethodGet, returnURL, nil)
	privateW := httptest.NewRecorder()
	srv.ServeHTTP(privateW, privateReq)
	if privateW.Code != http.StatusSeeOther {
		t.Fatalf("private response status = %d, want %d", privateW.Code, http.StatusSeeOther)
	}
	unauthenticatedCookie := responseCookieForTest(t, privateW.Result(), server.Sessions.CookieName)

	form := url.Values{}
	form.Set(common.ParamEmail, user.Email)
	form.Set(common.ParamPortalSolution, "captchaSolution")
	loginReq := httptest.NewRequest(http.MethodPost, "/"+common.LoginEndpoint, bytes.NewBufferString(form.Encode()))
	loginReq.Header.Set(common.HeaderContentType, common.ContentTypeURLEncoded)
	loginReq.AddCookie(unauthenticatedCookie)
	loginW := httptest.NewRecorder()
	server.postLogin(loginW, loginReq)
	if loginW.Code != http.StatusOK {
		t.Fatalf("login response status = %d, want %d", loginW.Code, http.StatusOK)
	}
	pendingCookie := responseCookieForTest(t, loginW.Result(), server.Sessions.CookieName)
	code := signInCodeForTest(t, user.Email)

	response := twoFactorSuite(srv, user.Email, server.XSRF.Token(pendingCookie.Value), code, pendingCookie)
	defer response.Body.Close()
	location, err := response.Location()
	if err != nil {
		t.Fatal(err)
	}
	if response.StatusCode != http.StatusSeeOther || location.RequestURI() != returnURL {
		t.Fatalf("post-login redirect = (%d, %q), want (%d, %q)", response.StatusCode, location.RequestURI(), http.StatusSeeOther, returnURL)
	}
}
