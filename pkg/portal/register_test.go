...
func TestRegistrationInvalidCSRFAfterConsume(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping integration test")
	}
	...
	resp = twoFactorSuite(srv, email, server.XSRF.Token(email), code, cookie)
	if resp.StatusCode != http.StatusSeeOther {
		t.Fatalf("consumed challenge replay status = %d, want redirect", resp.StatusCode)
	}
	if _, err := store.Impl().FindUserByEmail(t.Context(), email); err == nil {
		t.Fatal("consumed challenge replay created an account")
	}
}
...
func TestRegistrationCheckRequiresVerification(t *testing.T) {
	...
	resp = twoFactorSuite(srv, spammerEmail, server.XSRF.Token(spammerEmail), code, resp.Cookies()[idx])
	if resp.StatusCode != http.StatusSeeOther {
