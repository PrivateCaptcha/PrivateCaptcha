...
func TestApiGetProperty(t *testing.T) {
	...
	params := db_test.CreateNewPropertyParams(user.ID, "example.com")
	params.Challenge = dbgen.NullChallengeType{ChallengeType: dbgen.ChallengeTypeArgon2ID, Valid: true}
	property, _, err := server.BusinessDB.Impl().CreateNewProperty(ctx, params, org)
	if err != nil {
		t.Fatal(err)
	}
	...
	if output.Challenge != string(property.Challenge) {
		t.Errorf("Received property challenge %q, want %q", output.Challenge, property.Challenge)
	}
