...
func (v *Verifier) PuzzleForRequest(r *http.Request, levels *difficulty.Levels, rulesPair *rules.RulesPair, ri *rules.RequestInfo) (puzzle.Puzzle, *dbgen.Property, error) {
	...
	if accessErr := v.recordPuzzleAccess(ctx, levels, property, fingerprint, result, tnow, ri); err == nil {
		err = accessErr
	}
...
func (v *Verifier) recordPuzzleAccess(ctx context.Context, levels *difficulty.Levels, property *dbgen.Property, fingerprint common.TFingerprint, result puzzle.Puzzle, tnow time.Time, ri *rules.RequestInfo) error {
	...
	return levels.RecordAccess(ctx, accessRecord)
}
