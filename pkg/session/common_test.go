...
func TestManagerTransitionFailuresPreserveCookie(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "registration issue", run: func(w http.ResponseWriter) error {
			_, err := manager.IssueRegistrationChallenge(w, req, predecessor, 0, "user@example.com", "123456", 15*time.Minute, 5)
			return err
		}},
...
func TestExpirationRenewalWindowBoundary(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if actual := shouldScheduleExpirationRenewal(tt.authority, now, sessionExpirationRenewalWindow); actual != tt.want {
