...
func TestManagerIssueSignInChallengeSetsCookieOnlyAfterSuccess(t *testing.T) {
	pending := NewSessionWithAuthority(Authority{State: StatePending, Version: 1, ExpiresAt: time.Now().Add(3 * time.Hour)}, NewPayload(t.Name(), payloadStoreStub{}))
...
func TestManagerTransitionFailuresPreserveCookie(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "registration issue", run: func(w http.ResponseWriter) error {
			_, err := manager.IssueRegistrationChallenge(w, req, predecessor, "user@example.com", "123456", false, 15*time.Minute, 5)
			return err
		}},
...
func TestManagerConsumeSignInChallengeRotatesServerGeneratedSID(t *testing.T) {
	predecessor := NewSessionWithAuthority(Authority{State: StatePending, Version: 2, ExpiresAt: time.Now().Add(time.Hour)}, NewPayload("predecessor", payloadStoreStub{}))
	store := &stubStore{consumeSignIn: func(_ context.Context, consume SignInChallengeConsume) (*ChallengeResult, error) {
		successor := NewSessionWithAuthority(Authority{State: StateAuthenticated, Version: 1, UserID: 42, ExpiresAt: time.Now().Add(3 * time.Hour)}, NewPayload(consume.SuccessorSessionID, payloadStoreStub{}))
