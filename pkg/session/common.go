...
func (s *Session) PersistSignInChallenge(ctx context.Context, encodedCode, email string, challengeTTL time.Duration) error {
	s.data.set(KeyPersistent, true)
	if err := s.store.CreateSignInChallenge(ctx, s, encodedCode, email, challengeTTL); err != nil {
		s.data.delete(KeyPersistent)
		return err
	}
	return nil
...
func (s *Session) PersistRegistrationChallenge(ctx context.Context, encodedCode, email string, challengeTTL time.Duration) error {
	s.data.set(KeyPersistent, true)
	if err := s.store.CreateRegistrationChallenge(ctx, s, encodedCode, email, challengeTTL); err != nil {
		s.data.delete(KeyPersistent)
		return err
	}
	return nil
...
type Store interface {
	...
	ConsumeSignInChallenge(ctx context.Context, current, successor *Session, encodedCode string, maxFailedAttempts int32) (SignInChallengeResult, error)
	ConsumeRegistrationChallenge(ctx context.Context, current, successor *Session, encodedCode string, maxFailedAttempts int32, allowConsumption bool) (RegistrationChallengeResult, error)
