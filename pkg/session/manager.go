...
func (m *Manager) setSessionCookie(w http.ResponseWriter, r *http.Request, sid string, maxAge int) {
	...
	if m.MaxLifetime > 0 {
		cookie.Expires = time.Now().Add(m.MaxLifetime)
	}
...
func (m *Manager) IssueSignInChallenge(
...
func (m *Manager) IssueRegistrationChallenge(
	w http.ResponseWriter,
	r *http.Request,
	sess *Session,
	inviteID int32,
...
func (m *Manager) ResendPendingChallenge(
	ctx context.Context,
	sess *Session,
	code string,
	challengeTTL time.Duration,
...
func (m *Manager) IssueEmailChangeChallenge(
	ctx context.Context,
	sess *Session,
	code string,
	challengeTTL time.Duration,
...
func (m *Manager) ScheduleExpirationRenewal(w http.ResponseWriter, r *http.Request, sess *Session) {
	...
	if !ok || !shouldScheduleExpirationRenewal(authority, time.Now(), m.expirationRenewalWindow()) {
		return
	}
...
func shouldScheduleExpirationRenewal(authority Authority, now time.Time, expirationRenewalWindow time.Duration) bool {
	return authority.State == StateAuthenticated && now.Before(authority.ExpiresAt) && !authority.ExpiresAt.After(now.Add(expirationRenewalWindow))
}
