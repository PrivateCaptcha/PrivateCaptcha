          ...
          <td>
            ...
            <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="margin:16px 0 24px;border-collapse:collapse;width:100%">
              ...
              <tr{{if .Alternate}} style="background-color:#f9f9f9"{{end}}>
                ...
                <td style="padding:12px 16px;border:1px solid #dddddd;font-size:14px;text-align:left" title="{{.Domain}}">{{if .Link}}<a href="{{.Link}}" style="color:#000000;text-decoration:none">{{truncate .Domain 24}} <span style="font-size:12px">&#8599;</span></a>{{else}}{{truncate .Domain 24}}{{end}}</td>
