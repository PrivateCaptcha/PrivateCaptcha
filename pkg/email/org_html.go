...
type OrgMemberJoinedContext struct {
	UserName         string
	MemberName       string
	MemberEmail      string
...
var (
	...
	OrgMemberJoinedTemplate = common.NewEmailTemplate("org-member-joined", orgMemberJoinedHTMLTemplate, orgMemberJoinedTextTemplate, emailFuncs)
)
...
<html dir="ltr" lang="en">
  ...
  >
    {{ $utm := .UTM | default "utm_medium=email&utm_source=org" }}
    <table
      align="center"
      width="100%"
      border="0"
...
PrivateCaptcha © {{.CurrentYear}} Intmaker OÜ`
	orgMemberJoinedHTMLTemplate = `<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html dir="ltr" lang="en">
  <head>
    <link rel="preload" as="image" href="{{.CDNURL}}/portal/img/pc-logo-dark.png" />
    <meta content="text/html; charset=UTF-8" http-equiv="Content-Type" />
  ...
  >
      ...
      <tbody>
        <tr style="width:100%">
          <td>
            ...
            <p style="font-size:16px;line-height:26px;margin:32px 0 16px">
              <strong>{{.MemberName}}</strong> (<a href="mailto:{{.MemberEmail}}">{{.MemberEmail}}</a>) has joined your <strong>{{.OrganizationName}}</strong> organization in Private Captcha.
            </p>
            <p style="font-size:16px;line-height:26px;margin:16px 0">
              Warmly,<br />The Private Captcha team
            </p>
	...
	orgMemberJoinedTextTemplate = `Hello{{if .UserName}} {{.UserName}}{{end}},
{{.MemberName}} ({{.MemberEmail}}) has joined your '{{.OrganizationName}}' organization in Private Captcha.
