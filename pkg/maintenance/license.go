...
var (
	...
	errLicenseConfigError = errors.New("license config error")
	errNoLicenseKeys      = errors.New("license keys not found")
...
func NewCheckLicenseJob(store db.Implementor, config common.ConfigStore, version string, quitFunc func(ctx context.Context), licenseValid *atomic.Bool) (common.PeriodicJob, error) {
	...
	if requireActivationKeys() {
		if len(keys) == 0 {
			return nil, errNoLicenseKeys
		}

		if len(LicenseURL) == 0 {
			return nil, errLicenseConfigError
		}
	}
	...
	return &checkLicenseJob{
		...
		keys:         keys,
		url:          LicenseURL,
		licenseKey:   config.Get(licenseConfigKey()),
