...
func (j *ScheduleReportsJob) scheduleWeeklyReports(ctx context.Context, tnow time.Time, usersLimit int32) error {
	...
	for iteration := 0; iteration < maxPaginationIterations; iteration++ {
		...
		for _, user := range users {
			...
			userAccountStats, found := accountStats[user.UserID]
			if !found || userAccountStats == nil {
...
func (j *ScheduleReportsJob) buildWeeklyReport(ctx context.Context, userID int32, from, mid, to time.Time, accountStats *common.UserReportAccountStats) (*email.UsageReportContext, error) {
	...
	if accountStats == nil {
		...
		if accountStats == nil {
			err := fmt.Errorf("weekly account report stats are missing for user %d", userID)
			slog.ErrorContext(ctx, "Failed to find weekly account report stats", "userID", userID, common.ErrAttr(err))
			return nil, err
		}
	}

	stats, err := j.TimeSeries.RetrieveWeeklyPropertiesReportStats(ctx, userID, from, mid, to, userReportOptions())
	if err != nil {
	...
	fillChanges(report, accountStats)
...
func (j *ScheduleReportsJob) buildMonthlyReport(ctx context.Context, userID int32, from, mid, to time.Time, accountStats *common.UserReportAccountStats) (*email.UsageReportContext, error) {
	...
	if accountStats == nil {
		...
		accountStats = statsByUser[userID]
		if accountStats == nil {
			err := fmt.Errorf("monthly account report stats are missing for user %d", userID)
			slog.ErrorContext(ctx, "Failed to find monthly account report stats", "userID", userID, common.ErrAttr(err))
			return nil, err
		}
	}

	stats, err := j.TimeSeries.RetrieveMonthlyPropertiesReportStats(ctx, userID, from, mid, to, userReportOptions())
	if err != nil {
...
func fillChanges(report *email.UsageReportContext, stats *common.UserReportAccountStats) {
...
func fillPropertyReportDetails(ctx context.Context, store db.Implementor, report *email.UsageReportContext, stats *common.UserReportStats, portalURL string, hasher common.IdentifierHasher) {
	if stats == nil || ((len(stats.Properties) == 0 || report.TotalRequests == 0) && len(stats.ProtectionCandidates) == 0) {
		return
	}
	...
	batch := make(map[int32]uint, len(props)+len(stats.ProtectionCandidates))
	if report.TotalRequests > 0 {
		for _, ps := range props {
			batch[ps.PropertyID] = 0
		}
	}
	for _, candidate := range stats.ProtectionCandidates {
	...
	highlights := make([]*email.ProtectionHighlight, 0, len(stats.ProtectionCandidates))
	for _, candidate := range stats.ProtectionCandidates {
		...
		highlights = append(highlights, &email.ProtectionHighlight{
			Name:      prop.Name,
			Link:      propertyDashboardURL(ctx, portalURL, hasher, prop) + "?" + report.UTM,
	...
	report.ProtectionHighlights = highlights
}
