...
const removeOrgMemberByID = `-- name: RemoveOrgMemberByID :exec
DELETE FROM backend.organization_users WHERE id = $1 AND org_id = $2
`

type RemoveOrgMemberByIDParams struct {
	ID    int32 `db:"id" json:"id"`
	OrgID int32 `db:"org_id" json:"org_id"`
}

func (q *Queries) RemoveOrgMemberByID(ctx context.Context, arg *RemoveOrgMemberByIDParams) error {
	_, err := q.db.Exec(ctx, removeOrgMemberByID, arg.ID, arg.OrgID)
	return err
}
