...
type registrationVerificationQuerier struct {
	*QuerierStub
	row *dbgen.Session
	err error
}
...
func (q *successfulRegistrationQuerier) ConsumeRegistrationChallenge(
	context.Context,
	*dbgen.ConsumeRegistrationChallengeParams,
...
func TestSessionWorkersPreserveRevocationTombstones(t *testing.T) {
	tests := map[string]func(*SessionStore, string, *session.Session){
		"publishExpiration": func(store *SessionStore, sid string, _ *session.Session) {
			store.publishExpirationRenewal(session.ExpirationRenewalResult{SessionID: sid, Version: 2, ExpiresAt: time.Now().Add(time.Hour)})
		},
		"evictExpiration": func(store *SessionStore, sid string, _ *session.Session) {
			store.evictExpirationRenewal(sid)
		},
		"publishPayload": func(store *SessionStore, sid string, tombstone *session.Session) {
			store.publishPayloadVersion(sid, 2, 3, tombstone.Payload())
		},
		"evictPayload": func(store *SessionStore, sid string, _ *session.Session) {
...
func TestSessionStoreRejectsMalformedRevocationResults(t *testing.T) {
	tests := map[string]*dbgen.RevokeSessionRow{
		"wrongState": {SessionID: "sid", State: dbgen.SessionStateAuthenticated, Version: 3},
		"wrongSID":   {SessionID: "other", State: dbgen.SessionStateRevoked, Version: 3},
	}
...
func TestSessionStoreSetsRegistrationVerificationInCachedAuthority(t *testing.T) {
	sid := t.Name()
	payload := session.NewPayload(sid, noopPayloadStore{})
	data, err := payload.Snapshot()
	if err != nil {
	...
	stored := &dbgen.Session{
		...
		ChallengeEmail:     Text("registrant@example.com"),
		VerifyRegistration: Bool(false),
	}
	querier := &registrationVerificationQuerier{QuerierStub: &QuerierStub{}, row: stored}
	business := NewBusinessWithQuerier(nil, querier, NewStaticCache[CacheKey, any](100, &CacheMissingValue{}))
	store := NewSessionStore(business, &sessionMetricsStub{})
	delayed := *stored
	original, _, err := store.publishTransitionSession(storedDBSession(stored))
	if err != nil {
		t.Fatal(err)
	}
	...
	updatedAuthority, ok := updated.Authority()
	if !ok || !updatedAuthority.VerifyRegistration {
		t.Fatalf("updated Authority = %+v, want registration verification", updatedAuthority)
	}
	if updated == original || updated.Payload() != original.Payload() {
		t.Fatal("Authority update did not replace the cached session while preserving its Payload")
	}
	if originalAuthority.VerifyRegistration {
		t.Fatal("Authority update mutated the original session snapshot")
	}
	if _, exact, err := store.publishTransitionSession(storedDBSession(&delayed)); err != nil {
		t.Fatal(err)
	} else if exact.VerifyRegistration {
		t.Fatal("cache merge changed the exact transition Authority")
	}
	updated, _ = store.sessionCache.GetIfPresent(sid)
	updatedAuthority, _ = updated.Authority()
	if !updatedAuthority.VerifyRegistration {
		t.Fatal("delayed same-version transition cleared registration verification")
	}
}
...
func TestSessionStoreSetsRegistrationVerificationOnNewerCachedAuthority(t *testing.T) {
	...
	newer := *stored
	newer.Version = 2
	newer.VerifyRegistration = Bool(false)
	if _, _, err := store.publishTransitionSession(storedDBSession(&newer)); err != nil {
		t.Fatal(err)
	}
...
func TestSessionStoreResolveUsesUnexpiredPendingCache(t *testing.T) {
	...
	row.ChallengeKind = dbgen.NullSessionChallengeKind{SessionChallengeKind: dbgen.SessionChallengeKindRegistration, Valid: true}
	row.ChallengeEmail = Text("pending@privatecaptcha.com")
	row.VerifyRegistration = Bool(true)
	refreshedRow := testLiveSessionRow(t, sid, 2, "pending", now.Add(time.Hour))
	refreshedRow.State = dbgen.SessionStatePending
	refreshedRow.ChallengeKind = dbgen.NullSessionChallengeKind{SessionChallengeKind: dbgen.SessionChallengeKindRegistration, Valid: true}
	refreshedRow.ChallengeEmail = Text("refreshed@privatecaptcha.com")
	refreshedRow.VerifyRegistration = Bool(true)
	q := newScriptedLiveSessionQuerier()
	q.enqueue(liveSessionResponse{row: row})
	q.enqueue(liveSessionResponse{row: refreshedRow})
	_, store := testResolveStore(q)
	...
	if !ok || authority.State != session.StatePending || authority.ChallengeEmail != "pending@privatecaptcha.com" || !authority.VerifyRegistration {
		t.Fatalf("warm pending Authority = %+v", authority)
	}
	leaseBoundary := now.Add(10 * time.Minute)
	if want := leaseBoundary; !authority.LeaseUntil.Equal(want) {
		t.Fatalf("pending LeaseUntil = %v, want %v", authority.LeaseUntil, want)
	}

	refreshed, err := store.resolve(t.Context(), sid, leaseBoundary)
	if err != nil {
		t.Fatal(err)
	}
	refreshedAuthority, ok := refreshed.Authority()
	if refreshed == pending || !ok || refreshedAuthority.Version != 2 || refreshedAuthority.ChallengeEmail != "refreshed@privatecaptcha.com" {
		t.Fatalf("refreshed pending session = (%p, %+v), want version 2", refreshed, refreshedAuthority)
	}
}
