...
ON CONFLICT (session_id) DO UPDATE SET
    ...
    registration_requires_verification = NULL,
...
VALUES (
    ...
    sqlc.arg(requires_verification), sqlc.narg(invite_id)
)
ON CONFLICT (session_id) DO UPDATE SET
    state = EXCLUDED.state,
    version = sessions.version + 1,
    ...
    challenge_email = EXCLUDED.challenge_email,
    challenge_expires_at = EXCLUDED.challenge_expires_at,
    registration_requires_verification = EXCLUDED.registration_requires_verification,
...
WHERE session_id = sqlc.arg(session_id)
  ...
  AND (
      ...
      OR registration_requires_verification = FALSE
  )
RETURNING
