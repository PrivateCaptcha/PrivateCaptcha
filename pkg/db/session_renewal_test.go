...
func (q *scriptedExpirationRenewalQuerier) RenewSessionExpirations(_ context.Context, params *dbgen.RenewSessionExpirationsParams) ([]*dbgen.RenewSessionExpirationsRow, error) {
...
func TestSessionExpirationRenewalPublishesReturnedExpirationOnly(t *testing.T) {
	...
	if authority.Version != 8 || authority.UserID != 42 || authority.State != session.StateAuthenticated || !authority.ExpiresAt.Equal(renewedAt) || !authority.LeaseUntil.Equal(leaseUntil) {
