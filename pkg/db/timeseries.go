...
(
	...
	for rows.Next() {
		...
		if err := rows.Scan(&rowType, &propertyID, &orgID, &currentRequests, &previousRequests, &timestamp, &verifications, &failedVerifications); err != nil {
...
func (m *MemoryTimeSeries) memoryReportStats(userID int32, from, mid, to time.Time, options common.UserReportOptions) (*common.UserReportStats, error) {
	...
	for key, counts := range candidates {
		...
		if counts.FailedVerifies >= options.SecurityEventMinimumDominantCount && failureRatio > options.SecurityEventRatioThreshold {
