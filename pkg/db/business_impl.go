...
func (impl *BusinessStoreImpl) inspectChallengeMiss(
	...
	if kind != session.ChallengeKindEmailChange &&
		record.State == dbgen.SessionStatePending &&
		(actualKind == session.ChallengeKindSignIn || actualKind == session.ChallengeKindRegistration) &&
		!record.SessionExpired && maxAttempts > 0 && record.FailedAttempts >= maxAttempts {
		return session.TransitionAttemptsExhausted, nil
	}
	if kind == "" {
		if record.State != dbgen.SessionStatePending || (actualKind != session.ChallengeKindSignIn && actualKind != session.ChallengeKindRegistration) {
		...
		if record.State != expectedState || actualKind != kind {
			return session.TransitionStale, nil
		}
	}
	...
	if maxAttempts > 0 && record.FailedAttempts >= maxAttempts {
		return session.TransitionAttemptsExhausted, nil
	}
	if kind == session.ChallengeKindRegistration && record.RegistrationRequiresVerification.Bool {
...
func (impl *BusinessStoreImpl) IssueRegistrationChallenge(ctx context.Context, params *dbgen.IssueRegistrationChallengeParams) (*session.ChallengeIssueResult, error) {
	if params == nil || params.SessionID == "" || !params.ChallengeEmail.Valid || params.ChallengeEmail.String == "" || !params.ChallengeCode.Valid || params.ChallengeCode.String == "" || params.Data == nil || !params.RequiresVerification.Valid || (params.InviteID.Valid && params.InviteID.Int32 <= 0) || params.SessionTtl <= 0 || params.ChallengeTtl <= 0 || params.MaxAttempts <= 0 {
		return nil, ErrInvalidInput
	}
	if impl.querier == nil {
		return nil, ErrMaintenance
