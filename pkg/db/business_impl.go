...
const sessionReconcileTimeout = 2 * time.Second
...
func (c *TxCache) SetEx(ctx context.Context, key CacheKey, t any, ttl, refresh time.Duration) error {
	c.set[key] = &txCacheArg{item: t, ttl: ttl, refresh: refresh}
	return nil
}
...
func (c *TxCache) Commit(ctx context.Context, cache common.Cache[CacheKey, any]) {
	...
	for key, value := range c.set {
		var err error
		if (value.ttl > 0) || (value.refresh > 0) {
			ttl := DefaultCacheTTL
			if value.ttl > 0 {
				ttl = value.ttl
			}
...
func (impl *BusinessStoreImpl) RevokeUserSession(ctx context.Context, sid string) (session.SessionRevocationResult, error) {
	...
	if row.UserID.Valid && row.PreviousState == dbgen.SessionStateAuthenticated {
...
func (impl *BusinessStoreImpl) CreateUserSignInChallenge(ctx context.Context, sd *session.SessionData, persistKey session.SessionKey, userID int32, encodedCode, email string, sessionTTL, challengeTTL time.Duration) error {
	...
	if err != nil {
		createErr := err
		reconcileCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), sessionReconcileTimeout)
		defer cancel()
...
func (impl *BusinessStoreImpl) ConsumeUserEmailChangeChallenge(ctx context.Context, sd *session.SessionData, expectedUserID int32, newEmail, encodedCode string, maxFailedAttempts int32) (session.EmailChangeChallengeResult, error) {
	...
	if (row.Consumed && row.Version != row.PreviousVersion+1) || (!row.Consumed && !row.AttemptsExhausted && row.Version != row.PreviousVersion+1) {
		return session.EmailChangeChallengeResult{}, ErrInvalidInput
	}
...
func validReconciledRegistration(row *dbgen.Session, params *dbgen.FinalizeRegistrationSessionParams) bool {
	return row != nil && row.SessionID == params.SessionID && row.State == dbgen.SessionStateAuthenticated && row.Version == params.ExpectedVersion+1 && row.UserID.Valid && row.UserID.Int32 == params.UserID && row.ExpiresAt.Valid && !row.ChallengeKind.Valid && !row.ChallengeCode.Valid && !row.ChallengeEmail.Valid && !row.ChallengeExpiresAt.Valid
}
...
type sessionRotationResult struct {
	rotated           bool
	predecessorUsable bool
}

func (impl *BusinessStoreImpl) RotateUserSession(ctx context.Context, predecessor, successor *session.SessionData, ttl time.Duration) (sessionRotationResult, error) {
	prequeryFailure := sessionRotationResult{predecessorUsable: true}
	if predecessor == nil || successor == nil || predecessor.ID() == "" || predecessor.ID() == successor.ID() || ttl <= 0 {
		return prequeryFailure, ErrInvalidInput
	}
	expectedVersion, predecessorPersisted := predecessor.Persistence()
	expectedUserID, hasUserID := predecessor.UserID()
	if !predecessorPersisted || expectedVersion <= 0 || !hasUserID {
		return prequeryFailure, ErrInvalidInput
	}
	successor.SetAuthoritativeUserID(expectedUserID)
	payload, _, successorPersisted, err := successor.PersistenceSnapshot()
	if err != nil {
		return prequeryFailure, err
	}
	if successorPersisted {
		return prequeryFailure, ErrInvalidInput
	}
	if impl.querier == nil {
		return prequeryFailure, ErrMaintenance
	}

	params := &dbgen.RotateSessionParams{
		OldSessionID:    predecessor.ID(),
		ExpectedVersion: expectedVersion,
		ExpectedUserID:  expectedUserID,
		NewSessionID:    successor.ID(),
		SuccessorData:   payload,
	...
	row, err := impl.querier.RotateSession(ctx, params)
	if errors.Is(err, pgx.ErrNoRows) {
	...
	if err != nil {
		...
		reconcileCtx, cancel := context.WithTimeout(context.WithoutCancel(ctx), sessionReconcileTimeout)
		defer cancel()
		reconciledSuccessor, successorErr := impl.querier.GetSessionByID(reconcileCtx, successor.ID())
		reconciledPredecessor, predecessorErr := impl.querier.GetSessionByIDUnfiltered(reconcileCtx, predecessor.ID())
		if successorErr != nil || predecessorErr != nil || !validReconciledRotation(reconciledSuccessor, params) || !validReconciledRotationPredecessor(reconciledPredecessor, params) {
			return sessionRotationResult{}, rotateErr
		}
		successor.SetAuthority(session.StateAuthenticated, reconciledSuccessor.Version, reconciledSuccessor.ExpiresAt.Time, time.Time{})
		return sessionRotationResult{rotated: true}, nil
	}
	if !validRotation(row, params) {
		return sessionRotationResult{}, ErrInvalidInput
	}
	successor.SetAuthority(session.StateAuthenticated, row.Version, row.ExpiresAt.Time, time.Time{})
	return sessionRotationResult{rotated: true}, nil
}

func validRotation(row *dbgen.RotateSessionRow, params *dbgen.RotateSessionParams) bool {
	return row != nil && row.SessionID == params.NewSessionID && row.State == dbgen.SessionStateAuthenticated && row.Version == 1 && row.UserID.Valid && row.UserID.Int32 == params.ExpectedUserID && row.ExpiresAt.Valid && !row.ChallengeKind.Valid && !row.ChallengeCode.Valid && !row.ChallengeEmail.Valid && !row.ChallengeExpiresAt.Valid && row.FailedAttempts == 0 && bytes.Equal(row.Data, params.SuccessorData)
}

func validReconciledRotation(row *dbgen.Session, params *dbgen.RotateSessionParams) bool {
	return row != nil && row.SessionID == params.NewSessionID && row.State == dbgen.SessionStateAuthenticated && row.Version == 1 && row.UserID.Valid && row.UserID.Int32 == params.ExpectedUserID && row.ExpiresAt.Valid && !row.ChallengeKind.Valid && !row.ChallengeCode.Valid && !row.ChallengeEmail.Valid && !row.ChallengeExpiresAt.Valid && row.FailedAttempts == 0 && bytes.Equal(row.Data, params.SuccessorData)
}

func validReconciledRotationPredecessor(row *dbgen.Session, params *dbgen.RotateSessionParams) bool {
	return row != nil && row.State == dbgen.SessionStateRevoked && row.Version == params.ExpectedVersion+1 && row.UserID.Valid && row.UserID.Int32 == params.ExpectedUserID
}
...
func (impl *BusinessStoreImpl) StoreUserSessions(ctx context.Context, batch map[string]uint, persistKey session.SessionKey) error {
	...
	for _, sd := range cached {
		...
		if !sd.Has(persistKey) {
			slog.WarnContext(ctx, "Persistent session is missing persist key", common.SessionIDAttr(sd.ID()))
		}
...
func (impl *BusinessStoreImpl) RetrieveOrgProperties(ctx context.Context, org *dbgen.Organization, sort OrgPropertiesSort, offset, limit int) ([]*dbgen.Property, bool, error) {
	if (offset < 0) || (offset > math.MaxInt32) || (limit <= 0) {
...
// NOTE: see note for RetrieveDifficultyRulesByPropertyIDs
...
func (impl *BusinessStoreImpl) DeleteDifficultyRule(ctx context.Context, org *dbgen.Organization, rule *dbgen.DifficultyRule, user *dbgen.User) (*common.AuditLogEvent, error) {
	...
	params := &dbgen.DeleteDifficultyRuleParams{
		ID:        rule.ID,
		CreatorID: Int(user.ID),
