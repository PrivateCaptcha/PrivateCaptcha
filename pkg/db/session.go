...
func (ss *SessionStore) publishTransitionSession(stored *session.StoredSession) (*session.Session, session.Authority, error) {
	...
	published, _ := ss.sessionCache.Compute(stored.SessionID, func(current *session.Session, found bool) (*session.Session, otter.ComputeOp) {
		...
		// Screening is monotonic within a transition version, so delayed pre-screening rows cannot clear it.
		if hasCurrentAuthority && currentAuthority.Version == candidateAuthority.Version &&
			currentAuthority.State == session.StatePending && candidateAuthority.State == session.StatePending &&
			currentAuthority.ChallengeKind == session.ChallengeKindRegistration && candidateAuthority.ChallengeKind == session.ChallengeKindRegistration &&
			currentAuthority.VerifyRegistration {
			candidateAuthority.VerifyRegistration = true
		}
		return session.NewSessionWithAuthority(candidateAuthority, current.Payload()), otter.WriteOp
	})
	if published.IsRevoked() {
		return published, exactAuthority, ErrStaleSessionRead
	}
	return published, exactAuthority, nil
}
...
func (ss *SessionStore) IssueRegistrationChallenge(ctx context.Context, issue session.RegistrationChallengeIssue) (*session.ChallengeResult, error) {
	...
	result, err := ss.store.Impl().IssueRegistrationChallenge(ctx, &dbgen.IssueRegistrationChallengeParams{
		...
		Data: issue.Payload, InviteID: inviteID,
...
func (ss *SessionStore) ConsumeRegistrationChallenge(
	ctx context.Context,
	consume session.RegistrationChallengeConsume,
) (*session.RegistrationConsumeResult, error) {
