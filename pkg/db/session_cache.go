...
func authorityFromStored(stored *session.StoredSession, readTime time.Time) session.Authority {
	...
	if stored.State == session.StatePending || stored.State == session.StateAuthenticated {
	...
	return session.Authority{
		...
		VerifyRegistration: stored.VerifyRegistration,
...
func isUsableCachedSession(sess *session.Session, now time.Time) bool {
	...
	return (authority.State == session.StatePending || authority.State == session.StateAuthenticated) &&
		now.Before(authority.LeaseUntil)
}
