...
Authority and payload are separate in memory. Authority decides whether a session can authenticate or requires additional registration verification. Payload holds best-effort UI continuation data such as a name, return URL, and notifications. Payload cannot change identity, challenge state, expiration, or revocation. See [`Authority` and `Payload`](../pkg/session/model.go).
...
Pending and authenticated sessions have a fixed 10-minute validation lease, capped at session expiration. Inside the lease, requests trust the local cache. After it expires, the replica reads PostgreSQL and rejects missing, expired, revoked, disabled-user, or deleted-user authority. Database errors and stale reads fail closed. Same-version reads keep the local payload; newer versions replace it. See [`SessionStore.Resolve`](../pkg/db/session_cache.go).

## Typical Two-Replica Lifecycle

Replicas never synchronize directly. They meet through PostgreSQL.

Invite registration is stateless until form submission. The invite GET places the encrypted invite ID in a hidden field without creating an anonymous session. Any replica can decrypt and bounds-check that field on registration POST, then store the numeric ID with the pending challenge. After email verification, linking succeeds only when the organization row is still unlinked, has the registered email, and remains at the `invited` level.
...
Cached challenge data cannot authenticate a user. Every attempt is decided by a guarded PostgreSQL query, so replicas share expiration and the cumulative attempt count for that SID. Issuing another email-change challenge preserves the existing count and is rejected once the cap is reached.
...
- Failed attempts are cumulative per SID, not per user. A new captcha-backed registration or sign-in SID gets a new budget; an exhausted email-change budget requires a new authenticated session.
