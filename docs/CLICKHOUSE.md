...
Puzzle statistics are partitioned by the puzzle's UTC expiration date, not by arrival time. The design favors bounded late-arrival handling and retryable partition replacement over cross-table transactions.
...
The job is polled hourly but guarded by a two-hour distributed lock, so successful cluster-wide runs occur at most every two hours; failed runs release the lock for an earlier retry. Each run uses a two-hour ingestion grace period, drains up to three oldest daily partitions, then processes at most one month only when no eligible daily partition remains. Finalizer eligibility also uses ClickHouse's UTC clock, so its cutoff cannot force an open cohort closed. See [`PuzzleStatsFinalizationJob`](../pkg/maintenance/puzzle_stats.go) and its [registration](../cmd/server/main.go#L446).
...
- Outcome materialized views accept only expiration dates at least `UTC date(now - 1h)`. Daily finalization waits two hours, so events arriving after closure cannot recreate a dropped source partition; they intentionally do not change puzzle statistics.
...
- `puzzle_outcomes_recent` has no TTL, so daily replacement remains retryable while its source exists. Monthly recovery is bounded by the daily tables' one-year TTL; a backlog longer than that can lose unpublished history.
...
- Organization/user purges row-delete IP/UA daily and monthly targets because date partitions mix tenants. They do not delete outcomes or fingerprints, so a retained outcome partition can later restore deleted IP/UA rows. Property purges currently omit puzzle-stat tables. See [`lightDelete`](../pkg/db/timeseries.go#L1544).
