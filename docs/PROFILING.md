...
- start load test using `bin/loadtest -mode test-puzzle -env ./docker/pc.env.loadtest -duration 600 -rps 450 -sitekey-percent 70` (obviously you can play with args)
...
For testing `/verify` endpoint first you need to pre-generate `solutions.txt` file with solutions (requires `-solutions-file` argument with `-rps` and `-durations` set).
