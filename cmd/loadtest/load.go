...
func stubVerifyTargeter(host string, apiKey string, cfg common.ConfigStore) vegeta.Targeter {
	...
	return func(tgt *vegeta.Target) error {
		...
		tgt.URL = fmt.Sprintf("http:%s/%s", apiURLConfig.URL(), common.VerifyEndpoint)
		tgt.Body = []byte(
			"AQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA.Aaqqqqq7u8zM3d3u7u7u7u4AAAAAAAAAAAAQAAAAAAAAAAAAAAAAAAAAAAAAAAA=.AQCiRYnFLBXoqfEYUz7Up+ktTXhxXgw=",
		)
