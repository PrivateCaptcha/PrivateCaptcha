...
widget-size:
	@test -f widget/static/js/privatecaptcha.js
...
find-docker-2fa:
	@$(DOCKER) compose -f docker/docker-compose.base.yml exec -T postgres psql -U postgres -d privatecaptcha -Atc "SELECT challenge_code FROM backend.sessions WHERE challenge_code IS NOT NULL AND challenge_expires_at > NOW() ORDER BY challenge_expires_at DESC LIMIT 1"
